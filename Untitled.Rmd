...
numeroStruttureSardegna <- capacit2021 %>% 
  group_by(Tipologia, Comune) %>%
  summarise(`Numero Strutture` = max(`Numero Strutture`))#numero di mesi

#numeroStruttureSardegna 

numeroStruttureSardegna2= numeroStruttureSardegna %>%
  group_by(Tipologia) %>% 
  summarise(`Numero Strutture` = sum(`Numero Strutture`))


numeroStruttureSardegna2
...
filter(capacit2021) %>% 
  ...
  geom_point()+
  ggtitle("Camere e numero di strutture ricettive in base alla tipologia") +
  xlab("Camere") + ylab("Numero Strutture")
...
```{r }
summary(capacit2021$Letti)
summary(capacit2021$Camere)
summary(capacit2021$`Numero Strutture`)
#capacit2021[capacit2021$`Numero Strutture`==904]
capacit2021[capacit2021$`Numero Strutture` >= 404 ,] ## errore digitazione??
capacit2021[capacit2021$Letti >= 3000 ,]
...
capacit2021_mesi = filter(capacit2021) %>% #, Comune == "BARUMINI") %>%
  mutate(MonthLabel= month(Mese, label= T)) %>%
  group_by(Comune, MonthLabel) %>%
  summarise(Letti = sum(Letti))

capacit2021_mesi %>% 
  arrange(desc(Letti))
filter(capacit2021_mesi, Comune == "ALGHERO") %>% 
  #arrange(desc(Camere))%>% 
 # head(100) %>%
  ...
  theme(axis.text.y = element_text(size = 8), plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5)) +
  ggtitle("Numero posti letto nel comune di Alghero", 
          subtitle= "Numero di posti letto delle strutture ricettive per mese") +
  xlab("Mesi") + ylab("N° posti letto")
...
Il grafico mostra il numero dei posti letto durante i mesi
...
arrivi_per_provincia<- provenienza_cleaned %>% 
  group_by(provincia, mese) %>%
  #summarise(arrivi = sum(arrivi), presenze = sum(presenze ))

arrivi_per_provincia %>% 
  mutate(MonthLabel= month(mese, label= T)) %>%
  ggplot(aes(MonthLabel, arrivi, col= provincia))+
  geom_line() + 
  #geom_point() +
  scale_y_continuous(trans= "log2", labels = label_number( big.mark = ".", decimal.mark= ","))+
  scale_x_date()+
...
provenienza_cleaned %>% ggplot(aes(arrivi, fill=macro_provenienza))+
...
params <- provenienza_cleaned %>% filter(macro_provenienza=="Stranieri") %>%   summarize(mean = mean(arrivi), sd = sd(arrivi))

provenienza_cleaned %>% filter(macro_provenienza=="Italiani") %>%   ggplot(aes(sample = arrivi)) +   geom_qq(dparams = params) +  geom_abline()
